...
if (typeof firebase !== 'undefined') {
  ...
  function toggleDashboard() {
    if(userDashboard.style.display === 'none') {
  ...
  window.switchAuth = (mode) => {
    loginModal.style.display = mode === 'login' ? 'block' : 'none';
    registerModal.style.display = mode === 'register' ? 'block' : 'none';
    recoveryModal.style.display = mode === 'recovery' ? 'block' : 'none';
  };
  ...
  function loadUserData() {
    ...
    dbAvatar.innerHTML = `<img src="${user.photoURL || 'https://api.dicebear.com/7.x/initials/svg?seed=' + user.email}">`;
    dbUserName.textContent = user.displayName || 'Membro Premium';
    dbUserEmail.textContent = user.email;

    db.collection('users').doc(user.uid).get().then(doc => {
      if (doc.exists) {
        const data = doc.data();
        document.getElementById('dbName').value = data.nome || user.displayName || '';
        document.getElementById('dbTel').value = data.telefone || '';
  ...
  function renderOrders(list) {
    ...
    if(list.length === 0) {
      container.innerHTML = '<p class="empty-msg">Você ainda não realizou pedidos.</p>';
      return;
    }

    container.innerHTML = list.map((order, idx) => `
      <div class="db-order-card" style="padding: 1.5rem; background: var(--cream-lt); border-radius: var(--r-sm); border-left: 4px solid var(--bord);">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
          <strong>${order.tipo}</strong>
          <span style="font-size: 0.75rem; color: var(--text-muted);">${new Date(order.data).toLocaleDateString()}</span>
        </div>
        <p style="font-size: 0.85rem; margin-bottom: 1rem;">${order.tamanho ? 'Tamanho: ' + order.tamanho : ''} ${order.total ? '| R$ ' + order.total.toFixed(2) : ''}</p>
        <button onclick="repeatOrder(${idx})" class="btn-cro secondary sm">Repetir Pedido ♥</button>
      </div>
    `).join('');
  }
  ...
  window.repeatOrder = (idx) => {
    ...
    db.collection('users').doc(user.uid).get().then(doc => {
      ...
      if(order.tamanho) msg += `♥ Detalhes: ${order.tamanho}, ${order.massa}, ${order.recheio}\n`;
  ...
  if(dbAddressForm) dbAddressForm.addEventListener('submit', (e) => {
    ...
    db.collection('users').doc(user.uid).get().then(doc => {
      let addresses = [];
      if(doc.exists) addresses = doc.data().enderecos || [];
  ...
  function renderAddresses(list) {
    ...
    if(list.length === 0) {
      container.innerHTML = '<p class="empty-msg">Nenhum endereço cadastrado.</p>';
      return;
    }

    container.innerHTML = list.map((addr, idx) => `
      <div class="db-address-card" style="padding: 1.5rem; background: var(--cream-lt); border-radius: var(--r-sm); position: relative;">
        <strong>${addr.rua}, ${addr.num}</strong><br>
        <small>${addr.bairro} - ${addr.cidade} | ${addr.cep}</small>
        <button onclick="removeAddress(${idx})" style="position: absolute; top: 1rem; right: 1rem; background: none; border: none; color: #d32f2f; cursor: pointer;">Excluir</button>
      </div>
    `).join('');
  }
  ...
  auth.onAuthStateChanged(user => {
    if (user) {
      ...
      if(userBtn) userBtn.innerHTML = `<img src="${user.photoURL || 'https://api.dicebear.com/7.x/initials/svg?seed=' + user.email}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">`;
      
      // Auto-fill forms and dashboard
      db.collection('users').doc(user.uid).get().then(doc => {
        if (doc.exists) {
          const data = doc.data();
          ...
          if(telInput) telInput.value = data.telefone || '';
          
          if(msgInput && data.enderecos && data.enderecos.length > 0) {
            const def = data.enderecos[0];
            msgInput.placeholder = `Entrega para: ${def.rua}, ${def.num} (${def.bairro})...`;
          }
        }
      });
    } else {
